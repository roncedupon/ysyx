...
//每一行最后的斜杠表示在这一行和下一行是连着的---dest=rd,src1=R[rs1],src2=R[rs2],src1,2已经是读出来的
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 0111011", or       , R, R(dest) =SEXT(src1*src2,32));
  //I型指令--------------------------------------------------------------------------------------------------------------
  ...
  INSTPAT("???????????? ????? 100 ????? 0000011", lbu       , I, R(dest) = Mr(src1+SEXT(immI(s->isa.inst.val),8),1));
  INSTPAT("???????????? ????? 111 ????? 0010011", addi      , I, R(dest) = src1&immI(s->isa.inst.val));
  INSTPAT("???????????? ????? 100 ????? 0010011", xori      , I, R(dest) = src1^immI(s->isa.inst.val));
  INSTPAT("000000 ?????? ????? 001 ????? 0010011", slli      , I, R(dest) = src1<<SEXT(BITS(s->isa.inst.val,25,20),6));// TODO--这里不用SEXT可否?--测试后也对
  INSTPAT("000000 ?????? ????? 101 ????? 0010011", srli      , I, R(dest) = src1>>SEXT(BITS(s->isa.inst.val,25,20),6));// TODO--这里不用SEXT可否?--测试后也对
